...
const getFrontendUrl = () => {
  ...
  if (process.env.RAILWAY_ENVIRONMENT === 'production') {
    return 'https://your-vercel-app.vercel.app'; // Replace with your actual Vercel URL
  }
...
// Helper function to save game to database
...
app.get('/api/games/active/:userId', async (req, res) => {
  try {
    ...
    for (const [gameId, game] of activeGames) {
      // Only return if game is not over
...
app.get('/api/debug/active-games', (req, res) => {
  const games = [];
  for (const [gameId, game] of activeGames) {
    games.push({
      gameId: gameId,
...
io.on('connection', (socket) => {
  ...
  socket.on('sync-time', ({ gameId }) => {
    ...
    if (game) {
      socket.emit('time-sync', {
        ...
        activeTimer: game.activeTimer || 'white'
      });
    }
  });
  ...
  socket.on('find-match', async () => {
    ...
    console.log(`📝 Waiting queue size: ${waitingPlayers.length}`);
    ...
    if (waitingPlayers.length >= 2) {
      ...
      const gameData = {
        ...
        whiteSocket: p1.socketId,
        blackUser: p2.userId,
        blackUsername: p2.username,
        blackSocket: p2.socketId,
        moves: [],
      ...
      io.emit('live-game-update', {
        gameId: gameId,
        whitePlayer: p1.username,
        blackPlayer: p2.username,
        lastMove: null,
        moveCount: 0,
        fen: chessGame.fen(),
        isGameOver: false
  ...
  socket.on('make-move', async ({ gameId, from, to, whiteTime, blackTime, activeTimer }) => {
    const game = activeGames.get(gameId);
    if (!game) {
    ...
    try {
      ...
      if (move) {
        ...
        io.to(opponentSocket).emit('opponent-move', { 
          from, to, fen, 
          whiteTime: game.whiteTime, 
          blackTime: game.blackTime, 
          activeTimer: game.activeTimer 
        ...
        io.to(`spectator_${gameId}`).emit('spectator-update', {
          gameId: gameId,
          fen: fen,
          moves: game.moves,
          lastMove: `${from}→${to}`
  ...
  socket.on('disconnect', () => {
    ...
    if (socket.user._id !== 'spectator') {
      User.findByIdAndUpdate(socket.user._id, { online: false }).catch(console.error);
    }
    waitingPlayers = waitingPlayers.filter(p => p.socketId !== socket.id);
  });
});
